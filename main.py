...
class Handler(webapp2.RequestHandler):
    ...
    def set_sec_cookie(self, name, val):
        val = make_secure_val(val)
        self.response.headers.add_header('Set-Cookie',
                                         '{}={}; Path=/'.format(name, val))
...
class SimpleBlog(Handler):
    ...
    def get(self):
        blog_posts = db.GqlQuery("SELECT * FROM Post "
                                 "ORDER BY created DESC LIMIT 10")
        posts_comments = db.GqlQuery("SELECT * FROM Comment "
...
class NewPost(Handler):
    ...
    def post(self):
        ...
        if content and subject:
            ...
            # To look up an object by ID, you can use the get_by_id()
            # functionself.
            self.redirect('/blog/{}'.format(post.key().id()))
        else:
            error = "We need both, a valid subject (min. 3 char.) and a Blog!"
            self.render("blog_new_post.html",
                        error=error,
                        content=content,
                        subject=subject)
    ...
    def get(self, post_id):
        ...
        if self.user:
            if post.author.name == self.user.name:
                ...
                return self.render('login_form.html', error=error)
        else:
...
class LikingPost(Handler):
    def post(self, post_id):
        ...
        # really should use that Constructer for log.

        if self.user:
            # Once user voted he should not be able to change the votes
            if self.user.name in post.voters:
                return self.redirect("/blog")

            post.likes += 1
            # Is user name unique in the code or is appending ID better idea?
            post.voters.append(self.user.name)
            # This checks if the user is not the author
            # so the change changes the instance in DB
...
class DislikingPost(Handler):
    def post(self, post_id):
        ...
        if self.user:
            ...
            # Is user name unique in the code or is appending ID better idea?
            post.voters.append(self.user.name)
            # This checks if the user is not the author so the change
            # changes the instance in DB
            if self.user.name != post.author.name:
                post.put()
                self.redirect("/blog")
    ...
    def post(self, post_id):
        ...
        if self.user:
            if comment_author and comment and comment_id:
                ...
                return self.render("add_comment.html",
                                   post=post,
                                   subject=post.subject,
                                   content=p.content,
                                   error=error)
...
class EditComment(Handler):
    ...
    def get(self, comment_id):
        ...
        if self.user:
            ...
            return self.render('login_form.html',
                               error=error)
